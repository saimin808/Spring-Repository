	...
	form.addEventListener('submit', (e) => {
		...
		for(j = 0; j < elements.length; ++j) {
			...
			&& element.value.length <= 4) {
				window.alert('ID must be longer than 4');
				return;
			} else if (((element.name.includes('writer') || element.name.includes('writer'))
			&& element.value.length > 4)) {
				const is_valid = id_regex.test(element.value);
				if(is_valid == false) {
					window.alert('ID must start with eng letter \n'
						+ 'and must contains at least one letter and one number');
					return;
				}	
			}*/
